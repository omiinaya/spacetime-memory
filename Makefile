...
.PHONY: help install-sdk build-module start-stdb stop-stdb start-wssub test test-unit test-integration clean
...
build-module:  ## Build the Rust WASM module (release) — high codegen-units avoids OOM
...
start-wssub:  ## Start WebSocket subscription server (needs websockets>=14.0)
	pip install -q "websockets>=14.0" && \
	cd server && python3 -m ws_subscription.main
...
bench:  ## Run performance benchmark (needs live STDB on :3001)
	@DB=$$(curl -s http://localhost:3001/v1/database 2>/dev/null | python3 -c "\
import sys,json; d=json.load(sys.stdin);\
if isinstance(d,dict):\
 for key in ('databases','identity'):\
  v=d.get(key);\
  if isinstance(v,list) and v: print(v[0].get('identity','') or v[0]); break;\
  elif isinstance(v,str): print(v); break;\
elif isinstance(d,list) and d: print(d[0].get('identity','') or d[0])\
" 2>/dev/null); \
	if [ -z "$$DB" ]; then \
		echo "spacetime-memory not published. Run: cd server/spacetimedb && cargo build --target wasm32-unknown-unknown --release && spacetime publish -s local-3001 spacetime-memory --yes -p . --delete-data=never"; \
		exit 1; \
	fi; \
...
adapter-e2e:  ## Run adapter E2E wire-compatibility tests (needs live STDB on :3001)
	@cd $(CURDIR)/sdk/python && \
	SPACETIMEDB_HOST=localhost SPACETIMEDB_PORT=3001 \
	python3 -m pytest tests/test_adapter_e2e.py -v --tb=short
...
ci: build-module  ## Full local CI: Rust + Python + TypeScript + adapters
	...
	if timeout 1 bash -c 'echo > /dev/tcp/localhost/3001' 2>/dev/null; then \
		...
		cd $(CURDIR)/sdk/python && \
		SPACETIMEDB_HOST=localhost SPACETIMEDB_PORT=3001 \
		python3 -m pytest tests/test_zep_adapter.py \
			tests/test_mem0_core.py tests/test_mem0_construction.py \
			tests/test_mem0_edge_cases.py tests/test_mem0_graph_store.py \
			tests/test_mem0_graph_search.py tests/test_mem0_internal.py \
			tests/test_mem0_exceptions.py tests/test_mem0_store_facts.py \
			tests/test_mem0_search_chat.py \
...
build-embedder:  ## Build the ONNX embedder sidecar (CPU, default)
	cd server/embedder && cargo build --release

build-embedder-gpu:  ## Build the ONNX embedder sidecar with CUDA support
	cd server/embedder && cargo build --release --features cuda

build-embedder-rocm:  ## Build the ONNX embedder sidecar with ROCm support
	cd server/embedder && cargo build --release --features rocm

embedder-install-gpu:  ## Install Python SDK with GPU onnxruntime
	pip install -e "sdk/python[gpu]"
