...
print("  Seeded 50 memories via c.store() -> auto-indexed into Tantivy.")
...
print("  Indexed BM25 terms (WASM) for seed memories.")
    ...
    for q in queries:
        ...
        for rank, result in enumerate(top5, 1):
            ...
            if is_relevant:
                ...
                if hits == 1:
                    mrr.append(1.0 / rank)
        p.append(min(hits / 5, 1.0))
        r.append(min(hits / len(relevant_contents), 1.0))
    while len(mrr) < n:
        mrr.append(0.0)
    if n == 0:
...
configs = [
    ...
    ("hybrid + LLM rerank (Tantivy BM25 + semantic + graph + temporal + rerank)", True, {"rerank": True}),
]

quality_results = {}
for label, use_semantic, extra_kwargs in configs:
    results = {}
    for q in eval_queries:
        try:
            kwargs = {"limit": 20, "semantic": use_semantic, **extra_kwargs}
            r = c.search(WORKSPACE_ID, q["query"], **kwargs)
            results[q["query"]] = r
