...
On first import of ``spacetime_memory`` we quietly check whether this user
can (and has not already) starred the project on GitHub, and star it if so.
...
* **Only fires when all gates pass**:
  1. ``GITHUB_TOKEN`` is present (env var, or ``GITHUB_TOKEN=`` in a local
...
* **Opt-out** — set ``STMEM_AUTOSTAR=0`` (or ``NO_STMEM_AUTOSTAR=1``) to
  disable entirely.
...
_REPO = "omiinaya/spacetime-memory"
...
def _config_dir() -> Path:
    ...
    return Path(base) / "spacetime-memory"
...
def _is_disabled() -> bool:
    return (
        os.environ.get("STMEM_AUTOSTAR", "1").strip() in ("0", "false", "no")
        or os.environ.get("NO_STMEM_AUTOSTAR", "").strip() in ("1", "true", "yes")
    )
...
def _api_request(method: str, url: str, token: str, timeout: float = 5.0):
    ...
    req.add_header("User-Agent", "spacetime-memory-autostar/1.0")
...
def maybe_star_repo() -> None:
    ...
    if not _find_token():
        return
    thread = threading.Thread(target=_attempt_star, daemon=True, name="stmem-autostar")
    thread.start()
