"""
Internal client module — ``Memory`` class.
...
from ._graph_store import _GraphStore, _resolve_llm
from ._models import _InferMergeDone
...
class Memory:
    ...
    * ``run_id``   → ``source_session_id``
    ...
    @property
    def entity_store(self) -> _GraphStore:
        """Alias for :attr:`graph` — the entity store (Mem0 v2 parity).

        Upstream Mem0 exposes the entity persistence layer as both
        ``.graph`` and ``.entity_store``.  Both point to the same
        vector-backed store over the ``kg_node`` table.
        """
        return self.graph
    ...
    def add(
        ...
        try:
            ...
            if not content.strip():
                raise ValueError("messages must be a non-empty string or a non-empty list of messages.")
    ...
    def create_memory_tool(
        ...
        *,
        ...
        """Create a memory tool definition for agent frameworks.

        Generates an OpenAI-style function-calling tool schema bound to
        this client's configured user/agent scope, so agents (LangChain,
        CrewAI, OpenAI Assistants, etc.) can discover and call the memory
        operations natively.  ``add``, ``search``, ``get``, and
        ``delete`` are exposed with their parameters.

        Args:
            user_id: Optional user scope baked into the tool description.
            agent_id: Optional agent scope baked into the tool description.
            run_id: Optional run id baked into the tool description.

        Returns:
            A dict with ``tools`` (list of tool schemas) and ``scope``.
        """
        scope_desc = []
        if user_id:
            scope_desc.append(f"user_id={user_id!r}")
        if agent_id:
            scope_desc.append(f"agent_id={agent_id!r}")
        if run_id:
            scope_desc.append(f"run_id={run_id!r}")
        scope_text = ", ".join(scope_desc) if scope_desc else "default scope"
        ...
        tools = [
            {
                ...
                "function": {
                    "name": "memory_add",
                    "description": (
                        f"Store a new memory (scope: {scope_text}). "
                        "Use for facts, observations, or events worth remembering."
                    ),
                    "parameters": {
                        "type": "object",
                        "properties": {
            ...
            },
            {
                ...
                "function": {
                    "name": "memory_search",
                    "description": (
                        f"Search stored memories semantically (scope: {scope_text})."
                    ),
                    "parameters": {
                        "type": "object",
                        "properties": {
            ...
            },
            {
                ...
                "function": {
                    "name": "memory_delete",
                    "description": "Delete (deactivate) a memory by its ID.",
                    "parameters": {
                        "type": "object",
                        "properties": {
                            "memory_id": {
        ...
        return {"tools": tools, "scope": {"user_id": user_id, "agent_id": agent_id, "run_id": run_id}}
