...
try:
    from langgraph.store.base import BaseStore, GetOp, ListNamespacesOp, Op, PutOp, Result, SearchOp
except ImportError:
    # Stubs defined at module bottom for fallback
    BaseStore = object  # type: ignore[assignment]
    # Sentinel classes — NOT object, so isinstance checks don't match all objects.
    # When langgraph is absent, the batch method falls through to the op_type
    # string-based dispatch, which handles namedtuples correctly.
    class _Op: ...
    class _GetOp: ...
    class _PutOp: ...
    class _SearchOp: ...
...
class StmemStore(BaseStore):
    ...
    # -------------------------------------------------------------------
    # Native async variants (LangGraph BaseStore async API)
    # -------------------------------------------------------------------
    ...
    async def aput(
        ...
        **kwargs: Any,
        ...
        """Async put — delegates to sync put via asyncio.to_thread."""
        import asyncio

        await asyncio.to_thread(self.put, namespace, key, value, index=index)
    ...
    async def aget(
        ...
        **kwargs: Any,
        ...
        """Async get — delegates to sync get via asyncio.to_thread."""
        import asyncio
    ...
    async def adelete(
        ...
        **kwargs: Any,
        ...
        """Async delete — delegates to sync delete via asyncio.to_thread."""
        import asyncio
    ...
    async def asearch(
        ...
        **kwargs: Any,
        ...
        """Async search — delegates to sync search via asyncio.to_thread."""
        import asyncio

        return await asyncio.to_thread(
            self.search, namespace_prefix, query=query, filter=filter, limit=limit, offset=offset
        )
...
class StmemChatMessageHistory:
    ...
    def messages(self) -> list:
        ...
        except (ValueError, KeyError, TypeError):  # LangChain message deserialization failure
    ...
    def add_messages(self, messages: Sequence) -> None:
        ...
        for msg in messages:
            ...
            except (AttributeError, TypeError):  # LangChain message serialization failure
