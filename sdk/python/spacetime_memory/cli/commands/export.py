...
def export() -> None:
    ...
    Examples:
      ...
      stmem export markdown ./my-vault/ --kg-json --workspace my-project
    """
...
@click.option("--kg-json", is_flag=True,
              help="Also export full KG (nodes + edges) as kg.json")
def export_markdown(output_dir: str, workspace: str,
                    include_kg: bool, include_system: bool,
                    kg_json: bool) -> None:
    ...
    When --kg-json is set, a ``kg.json`` file is also written with
    the full knowledge graph (all nodes and edges) as a structured
    JSON document — ideal for external tooling and graph analysis.
    """
    ...
    msg = f"[green]Exported {files} files to {output_dir}/[/green]"
    if kg_json and result.get("kg_json_path"):
        msg += f"\n[green]  KG JSON: {result['kg_json_path']}[/green]"
    _quiet_print(msg)
