...
class BasePlugin(abc.ABC):
    ...
    def pre_llm_call(
        ...
        **kwargs: Any,
        ...
        """Called before every LLM call. Return (messages, model, kwargs).

        Lets a plugin inject system context, steer the model, or drop
        sensitive content from the request before it leaves the process.
        """
    ...
    def on_session_start(
        ...
        """Called when a new session/workspace is created.

        Return an optional dict of extra metadata to attach to the
        session; ``None`` means "no change".
        """
        return None
...
class PluginManager:
    ...
    def dispatch_pre_llm_call(
        self,
        messages: list[dict[str, str]],
        model: str,
        **kwargs: Any,
    ) -> tuple[list[dict[str, str]], str, dict[str, Any]]:
        """Run all pre_llm_call hooks in order.

        Returns (messages, model, kwargs) after every plugin has had a
        chance to transform the request.
        """
        for plugin in self._plugins:
            try:
                messages, model, kwargs = plugin.pre_llm_call(
                    messages, model, **kwargs
                )
            except Exception as e:
                logger.warning("Plugin %s.pre_llm_call failed: %s", plugin.name, e)
        return messages, model, kwargs
    ...
    def dispatch_session_start(
        ...
        """Run all on_session_start hooks, merging returned metadata.

        Returns the merged metadata dict (base metadata + plugin
        contributions, plugin values winning on key conflicts).
        """
        merged = dict(metadata or {})
        for plugin in self._plugins:
            try:
                extra = plugin.on_session_start(
                    session_id, workspace_id, dict(merged)
                )
                if isinstance(extra, dict):
                    merged.update(extra)
            except Exception as e:
                logger.warning("Plugin %s.on_session_start failed: %s", plugin.name, e)
        return merged
