...
def _mock_client():
    ...
    c._http.post.return_value = Mock(status_code=200, json=list)
    c.database = "test"
...
class TestGrantRevoke:
    ...
    def test_grant_with_empty_workspace_id(self):
        """Empty workspace ID passed through."""
        admin = _mock_client()
        result = admin._call("grant_space_access", ["", "user-1", "editor"])
        assert result == {"status": "ok"}

    def test_grant_with_empty_user_id(self):
        """Empty user ID passed through."""
        admin = _mock_client()
        result = admin._call("grant_space_access", ["ws-1", "", "editor"])
        assert result == {"status": "ok"}

    def test_revoke_with_empty_workspace(self):
        admin = _mock_client()
        result = admin._call("revoke_space_access", ["", "user-1"])
        assert result == {"status": "ok"}
...
class TestWorkspaceOperations:
    ...
    def test_create_workspace_empty_name(self):
        """Empty workspace name creates with empty string."""
        admin = _mock_client()
        result = admin._call("create_workspace", ["", "", "ws-empty"])
        assert result == {"status": "ok"}

    def test_update_nonexistent_workspace(self):
        """Updating non-existent workspace returns from reducer."""
        admin = _mock_client()
        admin._call.return_value = {"error": "workspace not found"}
        result = admin._call("update_workspace", ["ghost", "name", "desc"])
        assert "error" in result
...
class TestPermissionChecks:
    ...
    def test_set_initial_admin_twice(self):
        """Calling set_initial_admin twice doesn't crash."""
        admin = _mock_client()
        admin._call("set_initial_admin", ["admin-1"])
        admin._call("set_initial_admin", ["admin-1"])
        assert admin._call.call_count == 2
...
class TestErrorHandling:
    """ACL operations when reducer returns errors."""

    def test_store_reducer_error(self):
        """Store error propagated back."""
        admin = _mock_client()
        admin._call.return_value = {"error": "permission denied"}
        r = admin.store("ws-other", "p1", "content", memory_type="experience")
        assert "error" in r

    def test_grant_reducer_error(self):
        """Grant error propagated back."""
        admin = _mock_client()
        admin._call.return_value = {"error": "user not found"}
        result = admin._call("grant_space_access", ["ws-1", "ghost", "editor"])
        assert "error" in result

    def test_promote_reducer_error(self):
        """Promote error propagated back."""
        admin = _mock_client()
        admin._call.return_value = {"error": "cannot promote: already admin"}
        result = admin._call("promote_admin", ["already-admin"])
        assert "error" in result

    def test_demote_last_admin(self):
        """Demoting the last admin returns error from reducer."""
        admin = _mock_client()
        admin._call.return_value = {"error": "cannot demote: only admin"}
        result = admin._call("demote_admin", ["only-admin"])
        assert "error" in result

    def test_create_workspace_empty_name_allowed(self):
        """Even empty workspace name is passed through."""
        admin = _mock_client()
        r = admin._call("create_workspace", ["", "", "ws-id"])
        assert r == {"status": "ok"}
