"""Tests for the SDK Client — core functionality."""
...
class TestClientInit:
    ...
    def test_init_default_database(self):
        ...
        # Clear env var to test the code default, not any env override
        import os as _os
        old_db = _os.environ.pop("SPACETIMEDB_DB", None)
        try:
            client = Client(host="h", port="1")
            # Default is a 64-char hex string from _base.py
            assert len(client.database) == 64, f"got len={len(client.database)}"
            assert all(c in "0123456789abcdef" for c in client.database)
        finally:
            if old_db is not None:
                _os.environ["SPACETIMEDB_DB"] = old_db
