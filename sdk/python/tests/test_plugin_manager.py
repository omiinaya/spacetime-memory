...
# ═══════════════════════════════════════════════════════════════════
# Lifecycle hooks: pre_llm_call + on_session_start (Hermes parity)
# ═══════════════════════════════════════════════════════════════════


class _LLMInjectPlugin(BasePlugin):
    name = "llm-inject"
    version = "0.1.0"
    ...
    def pre_llm_call(self, messages, model, **kwargs):
        self.calls.append((messages, model, kwargs))
        messages = [{"role": "system", "content": "injected"}, *messages]
        return messages, "steered-model", {**kwargs, "max_tokens": 999}


class _SessionMetaPlugin(BasePlugin):
    name = "session-meta"
    version = "0.1.0"
    ...
    def on_session_start(self, session_id, workspace_id, metadata=None):
        self.calls.append((session_id, workspace_id, metadata))
        return {"plugin_attributed": True}


class TestPreLLMCallHook:
    def test_dispatch_runs_all_plugins_in_order(self):
        pm = PluginManager()
        p = _LLMInjectPlugin()
        pm.register(p)

        msgs, model, kwargs = pm.dispatch_pre_llm_call(
            [{"role": "user", "content": "hi"}], "base-model", temperature=0.1
        )

        assert len(p.calls) == 1
        assert model == "steered-model"
        assert msgs[0] == {"role": "system", "content": "injected"}
        assert kwargs["max_tokens"] == 999
        assert kwargs["temperature"] == 0.1

    def test_dispatch_faulty_plugin_skipped(self):
        pm = PluginManager()
        pm.register(FaultyPlugin())  # on_store raises, but not pre_llm_call

        class _FaultyLLM(BasePlugin):
            name = "faulty-llm"

            def pre_llm_call(self, messages, model, **kwargs):
                raise RuntimeError("boom")

        pm.register(_FaultyLLM())
        msgs, model, kwargs = pm.dispatch_pre_llm_call(
            [{"role": "user", "content": "hi"}], "m", temperature=0.0
        )
        assert msgs == [{"role": "user", "content": "hi"}]
        assert model == "m"
        assert kwargs["temperature"] == 0.0

    def test_base_plugin_noop(self):
        pm = PluginManager()
        pm.register(BasePlugin())
        msgs, model, kwargs = pm.dispatch_pre_llm_call(
            [{"role": "user", "content": "x"}], "m", temperature=0.5
        )
        assert model == "m"
...
class TestOnSessionStartHook:
    def test_dispatch_merges_metadata(self):
        pm = PluginManager()
        pm.register(_SessionMetaPlugin())

        merged = pm.dispatch_session_start(
            "ws-1", "ws-1", {"name": "default"}
        )

        assert merged["name"] == "default"
        assert merged["plugin_attributed"] is True

    def test_dispatch_no_metadata_default(self):
        pm = PluginManager()
        p = _SessionMetaPlugin()
        pm.register(p)

        merged = pm.dispatch_session_start("ws-1", "ws-1")

        assert p.calls[0][2] == {}
        assert merged == {"plugin_attributed": True}

    def test_base_plugin_noop_keeps_metadata(self):
        pm = PluginManager()
        pm.register(BasePlugin())

        merged = pm.dispatch_session_start("ws-1", "ws-1", {"a": 1})
        assert merged == {"a": 1}


class TestLLMRerankPluginWiring:
    """llm_rerank passes through the plugin_manager pre_llm_call hook."""

    def test_llm_rerank_invokes_plugin_hook(self):
        import json as _json

        from spacetime_memory.client._rerank import llm_rerank

        pm = PluginManager()
        pm.register(_LLMInjectPlugin())

        with patch("spacetime_memory.client._rerank.httpx.post") as mock_post:
            mock_resp = MagicMock()
                ...
                "choices": [
                    {
                        "message": {
                            "content": _json.dumps(
                                [{"index": 0, "score": 9.0, "reason": "relevant"}]
                            )
                        }
                    }
                ]
        ...
        # The plugin transformed the request: system message injected,
        # model steered, max_tokens=999
        body = mock_post.call_args[1]["json"]
        assert body["model"] == "steered-model"
        assert body["messages"][0] == {"role": "system", "content": "injected"}
