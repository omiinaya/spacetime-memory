"""Tests for SyncConnector base class internals (base.py).

Covers SyncConnector.__init__, _load_cursor, _save_cursor,
...
class TestSyncConnector(SyncConnector):
    """Concrete SyncConnector that returns canned events for testing."""
    __test__ = False  # pytest: helper class, not a test collection
...
# ── SyncConnector __init__ ───────────────────────────────────────────


class TestConnectorInit:
    """Tests for SyncConnector.__init__ behavior."""
    ...
    def test_custom_cursor_dir(self, tmp_path):
        ...
        d = tmp_path / "custom"
        conn = TestSyncConnector(cursor_dir=str(d))
        assert conn._cursor_dir == str(d)
        assert d.exists()  # os.makedirs was called
        assert conn._cursor_file == str(d / "TestSyncConnector_cursor.json")

    def test_logger_naming(self):
        """Logger is named connector.<ClassName>."""
        conn = TestSyncConnector()
        assert conn._log.name == "connector.TestSyncConnector"
...
class TestCursorLoad:
    ...
    def test_no_cursor_file(self, tmp_path):
        ...
        conn = TestSyncConnector(cursor_dir=str(tmp_path))
    ...
    def test_loads_valid_json(self, tmp_path):
        ...
        cursor_file = tmp_path / "TestSyncConnector_cursor.json"
...
class TestCursorSave:
    ...
    def test_save_overwrites_previous(self, tmp_path):
        ...
        data = json.loads((tmp_path / "TestSyncConnector_cursor.json").read_text())
        assert data == {"v": 2}
...
class TestRetryClientCall:
    ...
    def test_custom_max_retries(self):
        ...
        with mock.patch("time.sleep"), pytest.raises(httpx.RequestError):
...
class TestRun:
    ...
    def test_run_respects_max_per_tick(self):
        ...
        conn = TestSyncConnector(events=[Event(content=f"ev{i}") for i in range(10)])
        mock_client = mock.Mock()
    ...
    def test_run_stop_after_multiple_ticks(self):
        ...
        conn = TestSyncConnector(events=[Event(content="tick")])
        mock_client = mock.Mock()
    ...
    def test_run_handles_poll_error(self):
        ...
        conn = TestSyncConnector(poll_side_effect=RuntimeError("poll failed"))
        mock_client = mock.Mock()
    ...
    def test_run_handles_on_event_error(self):
        ...
        def crashing_on_event(event, client):
            ...
            super(TestSyncConnector, conn).on_event(event, client)
...
class TestConnectorRegistry:
    ...
    def test_poll_all_aggregates(self):
        reg = ConnectorRegistry()
        reg.register("a", TestSyncConnector(events=[Event(content="a1")]))
        reg.register("b", TestSyncConnector(events=[Event(content="b1"), Event(content="b2")]))
    ...
    def test_poll_all_handles_errors(self):
        reg = ConnectorRegistry()
        reg.register("ok", TestSyncConnector(events=[Event(content="ok")]))
        reg.register("bad", TestSyncConnector(poll_side_effect=RuntimeError("boom")))
...
class TestConnectorDaemon:
    ...
    def test_start_single_tick(self):
        ...
        with (
            ...
            mock_conn = TestSyncConnector(events=[Event(content="ev1")])
    ...
    def test_poll_error_handling(self):
        ...
        bad_conn = TestSyncConnector(poll_side_effect=RuntimeError("poll fail"))
    ...
    def test_event_handler_error_handling(self):
        ...
        # SyncConnector that returns events but raises in on_event
        conn = TestSyncConnector(events=[Event(content="ev1")])
