    ...
    def test_context_many_results(self, mock_client):
        ...
        mock_client._sql_rows = [
            {"entity_id": "m1", "entity_type": "memory", "score": 0.9,
             ...
             "embedding_json": json.dumps([0.1] * 384), "content": "auth login"},
            {"entity_id": "m2", "entity_type": "memory", "score": 0.8,
             "query_hash": "h2", "workspace_id": "ws-1", "context_json": ctx2,
             "embedding_json": json.dumps([0.1] * 384), "content": "db indexing"},
        ]
        mock_client._embed.return_value = [0.1] * 384
...
def mock_client():
    ...
    c._tantivy_circuit_open_until = 0.0
    c._tantivy_consecutive_failures = 0
    c.max_retries = 3
    c.plugin_manager = None
    c.event_bus = None
    c.embedder_url = "http://localhost:9090"
    c.tantivy_url = "http://localhost:9100"
    c._tantivy_search = MagicMock(return_value=[])
    c._boost_with_entity_signal = MagicMock(side_effect=lambda query, rows, workspace_id, boost_factor: rows)
    c._fuse_and_deduplicate = MagicMock(
        side_effect=lambda rows, tantivy_rows, per_strat, strat_min, strat_max, weights, **kwargs: [
            r for s in per_strat.values() for r in s
        ]
    )

    # _sql dispatches by query text: search_index queries return the
    # test-configured rows (set via c._sql_rows), memory trust-score
    # queries return rows with id/trust_score so the search pipeline
    # doesn't KeyError on `mr["id"]`.
    c._sql = MagicMock()
    c._sql_rows: list = []

    def _sql_side_effect(query: str, *args, **kwargs):
        if "FROM memory" in query and "trust_score" in query:
            return [{"id": "mem-1", "trust_score": 0.8}]
        if "FROM search_index" in query:
            return c._sql_rows
        return []

    c._sql.side_effect = _sql_side_effect
    return c
