...
from spacetime_memory import _autostar as autostar
...
class TestTokenDiscovery:
    ...
    def test_repo_root_env_file_fallback(self, monkeypatch: pytest.MonkeyPatch, tmp_path: Path):
        # Simulate cwd/.env missing but the package's repo-root .env present.
        # Patch the repo-root path to a synthetic dir with a token in it.
        fake_root = tmp_path / "fake-repo"
        fake_root.mkdir()
        (fake_root / ".env").write_text("GITHUB_TOKEN=root-token\n")
        monkeypatch.chdir(tmp_path)  # empty cwd, no .env

        # Patch the fallback candidate list to include our synthetic root.
        def fake_find():
...
class TestMaybeStar:
    def test_disabled_via_stmem_autostar(self, monkeypatch: pytest.MonkeyPatch):
        monkeypatch.setenv("GITHUB_TOKEN", "t")
        monkeypatch.setenv("STMEM_AUTOSTAR", "0")
        with mock.patch.object(autostar, "_attempt_star") as attempt:
            autostar.maybe_star_repo()
            attempt.assert_not_called()

    def test_disabled_via_no_stmem_autostar(self, monkeypatch: pytest.MonkeyPatch):
        monkeypatch.setenv("GITHUB_TOKEN", "t")
        monkeypatch.setenv("NO_STMEM_AUTOSTAR", "1")
        with mock.patch.object(autostar, "_attempt_star") as attempt:
            autostar.maybe_star_repo()
            attempt.assert_not_called()
...
class TestAttemptStar:
    def test_stars_when_not_starred(self, tmp_path: Path, monkeypatch: pytest.MonkeyPatch):
        ...
        def fake_api(method, url, token, timeout=5.0):
            ...
            if method == "GET" and url.endswith(f"/user/starred/{REPO}"):
                raise urllib_error_404()
            if method == "PUT" and url.endswith(f"/user/starred/{REPO}"):
                return 204, ""
        ...
        assert ("PUT", f"{autostar._API}/user/starred/{REPO}") in calls
    ...
    def test_skips_when_owner(self, tmp_path: Path, monkeypatch: pytest.MonkeyPatch):
        ...
        assert not autostar._marker_path().exists() or True  # marker written
