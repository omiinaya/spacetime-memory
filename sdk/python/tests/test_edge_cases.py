...
class TestNetworkPartition:
    ...
    @pytest.mark.unit
    def test_circuit_breaker_on_consecutive_failures(self, mock_http_client, monkeypatch):
        """Circuit breaker trips after consecutive failures.

        The breaker lives in ``_request_with_retry`` — once
        ``_consecutive_failures >= _circuit_breaker_threshold`` it opens
        and subsequent STDB calls raise immediately instead of hammering
        the server.
        """
        monkeypatch.setenv("OPENAI_API_KEY", "sk-test")
        ...
        with pytest.raises(RuntimeError, match=r"circuit breaker"):
            mock_http_client._request_with_retry(
                "POST", "http://localhost:3001/v1/database/test/sql",
                content="SELECT 1",
            )
