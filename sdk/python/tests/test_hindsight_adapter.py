...
from spacetime_memory.sdks.hindsight import (  # noqa: E402 — intentional: after pytestmark
    ...
    _run_async,
    _HindsightLowLevelShell,
    _empty_shell_response,
)
...
class TestCoverageGaps:
    ...
    def test_webhooks_shell(self, hindsight: Hindsight) -> None:
        """hindsight.webhooks returns LowLevelShell (line 430)."""
        wh = hindsight.webhooks
        assert isinstance(wh, _HindsightLowLevelShell)
        # Also test __call__ on the shell (line 279)
        result = wh("arg1", key="val")
        assert isinstance(result, dict)
        assert result.get("status") == "ok"

    def test_low_level_shell_call(self) -> None:
        """Direct call of _HindsightLowLevelShell (line 279)."""
        shell = _HindsightLowLevelShell("test_shell")
        result = shell("pos_arg", key="val")
        assert isinstance(result, dict)
        assert result.get("status") == "ok"
        assert "test_shell" in result.get("note", "")

    def test_empty_shell_response(self) -> None:
        """_empty_shell_response helper (line 282-292)."""
        result = _empty_shell_response("myshell", "mymethod")
        assert result["status"] == "ok"
        assert "myshell" in result["note"]
        assert "mymethod" in result["note"]
        assert result["items"] == []
