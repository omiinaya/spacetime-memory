...
from spacetime_memory.metrics import EndpointStats, MemoryStats, MetricsCollector
...
class TestMetricsCollector:
    ...
    def test_embedder_error_timestamps(self) -> None:
        """Embedder errors record timestamps for windowed rate calculation."""
        mc = MetricsCollector()
        assert mc.embedder_error_rate() == 0.0
        mc.record_embedder_error()
        mc.record_embedder_error()
        mc.record_embedder_error()
        # Within the window, should count all 3
        assert mc.embedder_error_rate() == 3.0
        # With a very large window, same
        assert mc.embedder_error_rate(600) == 3.0

    def test_embedder_error_rate_pct(self) -> None:
        """Embedder error rate percentage is computed correctly."""
        mc = MetricsCollector()
        # No calls yet — rate is 0
        assert mc.embedder_error_rate_pct() == 0.0
        # Add some endpoint calls
        mc.record("store", lambda: "ok")
        mc.record("search", lambda: "ok")
        mc.record_embedder_error()
        mc.record_embedder_error()
        # 2 errors in 2 calls = 100% (these are separate from embedder errors)
        # But our rate is based on endpoint calls, not embedder calls
        # The embedder errors don't count as endpoint calls
        assert mc.to_dict()["total_calls"] == 2
        # embedder_error_rate_pct = embedder_errors / total_endpoint_calls * 100
        assert mc.embedder_error_rate_pct() == 100.0  # 2 embedder errors / 2 calls
        # When there are calls, it's errors/total*100
        with patch("time.monotonic") as mock_time:
            mock_time.return_value = 1000
            mc2 = MetricsCollector()
            mc2.record("a", lambda: 1)
            mc2.record("b", lambda: 2)
            mc2.record_embedder_error()  # now
            mc2.record_embedder_error()  # now
            assert mc2.embedder_error_rate_pct() == 100.0

    def test_embedder_recovery_count(self) -> None:
        """Recovery count is tracked and exported."""
        mc = MetricsCollector()
        assert mc.to_dict()["embedder_recovery_count"] == 0
        mc.record_embedder_recovery()
        mc.record_embedder_recovery()
        assert mc.to_dict()["embedder_recovery_count"] == 2
        assert mc.to_dict()["embedder_error_rate_window_secs"] == 300

    def test_embedder_error_in_prometheus(self) -> None:
        """Prometheus output includes error rate gauge and recovery counter."""
        mc = MetricsCollector()
        mc.record_embedder_error()
        mc.record_embedder_error()
        mc.record_embedder_recovery()
        text = mc.prometheus_text()
        assert "spacetime_memory_embedder_error_rate_pct" in text
        assert "# TYPE spacetime_memory_embedder_error_rate_pct gauge" in text
...
class TestOtelMetricsBridge:
    ...
    def setup_method(self):
        """Skip all tests if opentelemetry-sdk < 1.43.0 (missing remove_metric_reader)."""
        try:
            import opentelemetry.sdk
            ver = tuple(int(x) for x in opentelemetry.sdk.__version__.split("."))
            if ver < (1, 43, 0):
                pytest.skip(f"opentelemetry-sdk {opentelemetry.sdk.__version__} < 1.43.0 — remove_metric_reader unavailable")
        except (ImportError, AttributeError, ValueError):
            pytest.skip("opentelemetry-sdk not found or version detection failed")
    ...
    def test_otel_disabled_by_env(self) -> None:
        ...
        import spacetime_memory.metrics as _m
        from spacetime_memory.metrics import is_otel_metrics_active, setup_otel_metrics
