...
class TestSlackGetChannelName:
    ...
    def test_timeout_error(self):
        """TimeoutError raises client.get — falls back to channel_id."""
        connector = SlackConnector(
            token="tok",
            channel_ids=["C1"],
            workspace_id="ws",
    ...
    def test_os_error(self):
        """OSError raises client.get — falls back to channel_id."""
        connector = SlackConnector(
            token="tok",
            channel_ids=["C1"],
            workspace_id="ws",
        ...
        client = Mock()
        client.get.side_effect = OSError("disk failure")
        name = connector._get_channel_name(client, "C1")
        assert name == "C1"
...
class TestSlackPollEmptyChannels:
    """poll() when no channels are configured."""

    def test_empty_channel_ids_returns_empty(self):
        """poll() with empty channel_ids list returns []."""
        connector = SlackConnector(
            token="tok",
            channel_ids=[],
            workspace_id="ws",
