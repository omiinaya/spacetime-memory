...
describe("Client", () => {
  ...
  describe("storeBatch", () => {
    ...
    it("passes images inline in batch payload context field", async () => {
      ...
      globalThis.fetch = vi.fn().mockImplementation(async (url: string, req?: any) => {
        ...
        if (url.includes("call/store_memory_batch") && req?.body) {
          batchPayload = JSON.parse(req.body);
        }
        if (url.includes("sql")) {
          return { ok: true, text: vi.fn().mockResolvedValue("[]") };
        }
      ...
      await client.storeBatch("ws-1", [
        { content: "First", images: "https://ex.com/a.png" },
        { content: "Second", images: ["https://ex.com/b.png", "https://ex.com/c.png"] },
        { content: "No image" },
      ]);

      expect(batchPayload).not.toBeNull();
      const items = JSON.parse(batchPayload[0]);
      expect(items.length).toBe(3);
      expect(items[0].context).toBe(JSON.stringify(["https://ex.com/a.png"]));
      expect(items[1].context).toBe(JSON.stringify(["https://ex.com/b.png", "https://ex.com/c.png"]));
      expect(items[2].context).toBe("");
    });
  });
  ...
  describe("sql injection protection", () => {
    ...
    it("queryGraph handles LIKE injection via % and _", async () => {
      mockSqlResponse([{ id: "n1", label: "test_%_input", node_type: "concept" }]);
      const nodes = await client.queryGraph("ws-1", "%_");
      expect(nodes).toHaveLength(1);
    });

    it("searchFacts handles LIKE injection via % and _", async () => {
      mockSqlResponse([{ content: "test_%_content" }]);
  ...
  describe("memory utilities", () => {
    ...
    it("batchDeleteMemories calls batch_delete_memories reducer", async () => {
      mockReducerOk();
      await client.batchDeleteMemories("ws1", ["mem-1", "mem-2"]);
      const [url, req] = (globalThis.fetch as any).mock.calls[0];
      expect(url).toContain("call/batch_delete_memories");
      expect(JSON.parse(req.body)).toEqual(["ws1", "[\"mem-1\",\"mem-2\"]"]);
    });

    it("batchDeleteMemories skips call for empty array", async () => {
      globalThis.fetch = vi.fn();
      await client.batchDeleteMemories("ws1", []);
      expect(globalThis.fetch).not.toHaveBeenCalled();
    });
