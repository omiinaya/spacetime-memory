...
export interface SearchOptions {
  ...
  /** Optional temporal filter (valid_from/valid_to time range). */
  temporalFilter?: { from?: number; to?: number };
  /** Optional Unix timestamp — only return results with created_at < before. */
  before?: number;
  /** Optional Unix timestamp — only return results with created_at > after. */
  after?: number;
}
...
/**
 * An LLM client that calls an external API (OpenAI-compatible) with custom config.
 * Used when the user provides provider/model/apiKey/baseUrl via llmConfig.
 */
class CustomLLMClient implements LLMClient {
  ...
  constructor(config: { provider?: string; model?: string; apiKey?: string; baseUrl?: string }) {
    ...
    // If a provider name is given without a baseUrl, resolve well-known providers
  ...
  async chat(messages: Array<{ role: string; content: string }>): Promise<string | null> {
    if (!this.apiKey) {
      console.warn("CustomLLMClient: no apiKey configured, returning null");
      return null;
    }
    ...
    try {
      ...
      if (!resp.ok) {
        console.warn(`CustomLLMClient: API returned ${resp.status}: ${await resp.text()}`);
        return null;
      }
      const data = await resp.json();
      return (data as any)?.choices?.[0]?.message?.content ?? null;
    } catch (err) {
      console.warn("CustomLLMClient: request failed", err);
      return null;
    }
  }

  private async _call(userPrompt: string): Promise<string | null> {
    return this.chat([{ role: "user", content: userPrompt }]);
  }
}
...
class GraphStore {
  ...
  async _ws(userId?: string): Promise<string> {
  ...
  async add(text: string, options: GraphAddOptions = {}): Promise<GraphEntity> {
    ...
    try {
      const semanticRows = await this.memory._client.search(wsId, cleaned, { limit: 5, semantic: true });
      ...
      for (const r of nodeRows) {
        ...
        if (!nid) continue;
        const rows = await this.memory._client._query("kg_node", wsId, { id: nid });
        if (!rows.length) continue;
        const existing = rows[0];
        const existingName = (existing.label as string) || "";
        try {
        ...
        return {
          id: nid,
          label: existingName,
          node_type: (existing.node_type as string) || entityType,
  ...
  async search(query: string, options: GraphSearchOptions = {}): Promise<GraphEntity[]> {
    ...
    try {
      const semanticRows = await this.memory._client.search(wsId, query, { limit, semantic: true });
      const nodeRows = semanticRows.filter((r) => r.entity_type === "node");
      if (nodeRows.length) {
    ...
    try {
      ...
      const matched = rows.filter((r) => ((r.entity_name as string) || "").toLowerCase().includes(q));
...
export class Memory {
  ...
  private _resolveLlmFor(userId?: string): LLMClient {
    if (userId && this._llmOverrides.has(userId)) {
      ...
      return new CustomLLMClient(cfg);
    }
  ...
  async _ws(userId?: string): Promise<string> {
    ...
    if (!this._userIdToWs.has(userId)) {
      // Check if a workspace for this user already exists
      try {
        const wsList = await this._client._call("list_workspaces", []);
        if (Array.isArray(wsList)) {
          // Try to match by name (userId is used as the workspace name)
          const existing = wsList.find((w: any) =>
            w && (w.name === userId || w.id === userId)
          );
          if (existing) {
            this._userIdToWs.set(userId, existing.id);
            return existing.id;
          }
        }
      } catch {
        // If listing fails, fall through to create + query
      }
      // Create workspace if no match found
      try {
  ...
  async add(
    ...
    try {
      ...
      if (finalUserId) {
        const stored = await this._client.search(wsId, content, { limit: 1, semantic: true });
        if (stored.length) {
      ...
      const searchResults = await this._client.search(wsId, content, { limit: 1, semantic: true });
  ...
  async search(
    ...
    try {
      const rows = await this._client.search(wsId, query, {
  ...
  async chat(
    ...
    if (llmConfig) {
      const llmClient = new CustomLLMClient(llmConfig);
      const llmMessages = [
    ...
    } else if (userId && this._llmOverrides.has(userId)) {
      const cfg = this._llmOverrides.get(userId)!;
      const llmClient = new CustomLLMClient(cfg);
      const llmMessages = [
        { role: "system", content: systemPrompt },
        ...(messages ? messages : []),
        { role: "user", content: query },
