...
#[table(accessor = graph_traversal_result)]
#[derive(Debug, Clone)]
...
#[table(accessor = shortest_path_result)]
#[derive(Debug, Clone)]
...
pub fn graph_bfs(
    ...
    // Pre-cleanup: remove stale results for this workspace_id + query_id
    for old in ctx.db.graph_traversal_result().iter()
        .filter(|r| r.workspace_id == workspace_id && r.query_id == qid)
        .collect::<Vec<_>>()
    {
        ctx.db.graph_traversal_result().id().delete(&old.id);
    }
...
#[table(accessor = kg_stats_result)]
#[derive(Debug, Clone)]
...
mod tests {
    ...
    #[test]
    fn test_bridge_score_special_characters() {
        // Simulate bridge score for node connecting special-char communities
        let community_count = vec!["comm!@#", "comm$%^"];
        let unique_count = community_count.iter().collect::<std::collections::HashSet<_>>().len() as f64;
        let total = 5.0;
        let score = if total > 1.0 {
            (unique_count - 1.0) / (total - 1.0)
        } else {
            0.0
        };
        assert!(score >= 0.0);
    ...
    #[test]
    fn test_bridge_score_unicode_ids() {
        let community_count = vec!["ノード", "コミュニティ"];
        let unique_count = community_count.iter().collect::<std::collections::HashSet<_>>().len() as f64;
        let total = 5.0;
        let score = if total > 1.0 {
            (unique_count - 1.0) / (total - 1.0)
    ...
    #[test]
    fn test_bridge_score_empty_communities() {
        let unique_count = 0.0;
        let total = 0.0;
        let score = if total > 1.0 {
            (unique_count - 1.0) / (total - 1.0)
        } else {
    ...
    #[test]
    fn test_kg_stats_very_large_values() {
        let node_count = 1_000_000u64;
        let edge_count = 100_000_000u64;
        let avg = if node_count > 0 {
            edge_count as f64 / node_count as f64
        } else {
            0.0
    ...
    #[test]
    fn test_community_count_unicode() {
        let community_ids = vec![1u64, 2, 3];
        let mut unique = std::collections::HashSet::new();
        for cid in community_ids {
            if cid > 0 {
                unique.insert(cid);
    ...
    #[test]
    fn test_orphan_nodes_empty_search() {
        let node_edges: std::collections::HashMap<String, u32> = [
            ("a".to_string(), 2u32),
        ].into();
        let all_nodes = vec!["a".to_string()];
        let orphans: Vec<_> = all_nodes.into_iter()
            .filter(|n| node_edges.get(n).map(|&e| e == 0).unwrap_or(true))
            .collect();
