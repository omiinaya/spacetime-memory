...
pub fn record_note_revision(
    ...
    let enc_new_title = encrypt_if_enabled(ctx, &note.workspace_id, new_title)?;
    let enc_new_content = encrypt_if_enabled(ctx, &note.workspace_id, new_content)?;
    ...
    let revision = NoteRevision {
        ...
        new_title: enc_new_title,
        new_content: enc_new_content,
...
#[table(accessor = backlink_result)]
#[derive(Debug, Clone, serde::Serialize, serde::Deserialize)]
...
pub fn create_note(
    ...
    trace_span!(ctx, "create_note", TracingSpanKind::Write, &workspace_id, {
        ...
        // Encrypt content if workspace encryption is enabled; keep plaintext for backlink parsing
...
pub fn update_note(
    ...
    trace_span!(ctx, "update_note", TracingSpanKind::Write, &ws_id, {
        ...
        // Encrypt content if workspace encryption is enabled; keep plaintext for backlink/block re-parsing
        let enc_content = encrypt_if_enabled(ctx, &note.workspace_id, &content)?;
...
/// Populates the public  table for SQL reads.
#[reducer]
pub fn get_backlinks(ctx: &ReducerContext, note_id: String) -> Result<(), String> {
    ...
    for bl in ctx.db.note_backlink().target_note_id().filter(&note_id) {
...
pub fn get_outgoing_links(ctx: &ReducerContext, note_id: String) -> Result<(), String> {
    ...
    for bl in ctx.db.note_backlink().source_note_id().filter(&note_id) {
        let source_title = note_titles.get(&bl.source_note_id).cloned().unwrap_or_default();
        let target_title = note_titles.get(&bl.target_note_id).cloned().unwrap_or_default();
        ctx.db.backlink_result().insert(BacklinkResult {
            id: uuid_v4_uniq(ctx, |id| ctx.db.backlink_result().id().find(id).is_none(), 3),
...
fn classify_block(block: &str) -> (String, u8, String, u32, String, String) {
    ...
    {
        let indent = count_indent(block);
        let content = trimmed.split_once(". ").map(|x| x.1).unwrap_or("").trim().to_string();
        return ("list_item".to_string(), 0, "none".to_string(), indent, "{}".to_string(), content);
    }
...
// ---------------------------------------------------------------------------
// Backlink helpers
/// Parse [[wikilinks]] in note content, create NoteBacklink rows, and bump
/// the target note's backlink_count. Targets are resolved by title within the
/// source note's workspace. Self-links are ignored.
fn resolve_backlinks(ctx: &ReducerContext, note_id: &str, content: &str, now: i64) {
    let Some(source) = ctx.db.note().id().find(note_id.to_string()) else {
        return;
    };
    let ws = source.workspace_id.clone();

    for (target_title, display) in find_wikilinks(content) {
        let target = ctx
            .db
            .note()
            .workspace_id()
            .filter(&ws)
            .find(|n: &Note| n.title == target_title && n.is_active);
        let Some(target) = target else { continue };
        if target.id == note_id {
            continue; // no self-links
        }
        let link_id = uuid_v4_uniq(ctx, |lid| ctx.db.note_backlink().id().find(lid).is_none(), 3);
        ctx.db.note_backlink().insert(NoteBacklink {
            id: link_id,
        ...
        let mut t = target;
        t.backlink_count = t.backlink_count.saturating_add(1);
        ctx.db.note().id().update(t);
    }
}

/// Find [[Title]] and [[Title|Display]] wikilinks in text.
/// Returns (title, display_text) pairs.
fn find_wikilinks(text: &str) -> Vec<(String, String)> {
            ...
            if let Some(end) = text[i + 2..].find("]]") {
                ...
                if !inner.is_empty() {
                    if let Some(pipe) = inner.find('|') {
                        ...
                        links.push((inner.to_string(), inner.to_string()));
                    }
                }
                i += 2 + end + 2;
                continue;
...
fn clear_backlinks(ctx: &ReducerContext, note_id: &str) {
    let old_links: Vec<_> = ctx
        .db
        .note_backlink()
        .source_note_id().filter(&note_id.to_string())
...
mod tests {
    ...
    fn test_note_empty_content() {
        let note = Note {
            id: "n_empty".to_string(),
            workspace_id: "ws".to_string(),
            title: String::new(),
            content: String::new(),
            note_date: String::new(),
        ...
        assert!(note.note_date.is_empty());
        assert!(note.is_active);
    }
    
    #[test]
    fn test_note_special_characters_in_content() {
        let content = "Hello! @#$%^&*()_+-=[]{}|;':\",./<>?`~ 😊".to_string();
        let note = Note {
            id: "n_special".to_string(),
            workspace_id: "ws".to_string(),
        ...
        assert_eq!(note.title, "Special!@#$%");
    }
    
    #[test]
    fn test_note_unicode_content() {
        let content = "日本語のノート系统测试 🎉 中文 漢字".to_string();
        let note = Note {
            id: "n_unicode".to_string(),
            workspace_id: "ws".to_string(),
            title: "Unicode Note テスト".to_string(),
            content: content.clone(),
            note_date: String::new(),
            embedding_json: String::new(),
            created_at: 0,
        ...
        assert_eq!(note.content, content);
        assert!(note.title.contains("テスト"));
        assert!(note.content.contains('🎉'));
    }
    
    #[test]
    fn test_note_very_large_content() {
        let large = "Note line\n".repeat(5000);
        let note = Note {
            id: "n_large".to_string(),
            workspace_id: "ws".to_string(),
        ...
        assert!(note.content.len() > 30_000);
        assert!(note.content.starts_with("Note line"));
    }
    
    #[test]
    fn test_note_concurrent_creation_simulation() {
        let notes: Vec<Note> = (0..10)
            .map(|i| Note {
                id: format!("n_concurrent_{}", i),
        ...
        for (i, note) in notes.iter().enumerate() {
            assert_eq!(note.id, format!("n_concurrent_{}", i));
        }
    }
    
    #[test]
    fn test_note_network_partition_simulation() {
        let note = Note {
            id: "n_partition".to_string(),
        ...
        assert!(note.workspace_id.is_empty());
        assert_eq!(note.title, "Orphan");
        assert_eq!(note.content, "Partial data");
        assert_eq!(note.backlink_count, 0);
    }
}
