...
pub struct KgNode {
    ...
    pub created_at: i64,
    /// Document ID that created this node (for ripple tracking)
    pub source_document_id: String,
    /// Micros timestamp when node became stale (0 = not stale)
    pub stale_since: i64,
}
...
pub fn create_node(
    ...
    // Register in WorkspaceIndex for hybrid_search graph-strategy pre-filtering
    hybrid_query::register_workspace_entity(ctx, &ws_id, "node", &id);
    Ok(())
}
...
pub fn update_node(
    ...
    // Encrypt summary if workspace encryption is enabled
    node.summary = encrypt_if_enabled(ctx, &node.workspace_id, &summary)?;
    ...
    // Reset stale flag on update (summary has been re-evaluated)
    node.stale_since = 0;
...
pub fn create_edge(
    ...
    // Register in NodeEdgeIndex for O(1) PK lookups during graph-strategy search
    hybrid_query::register_node_edge(ctx, &ws_id, &source_node_id, &id);
    hybrid_query::register_node_edge(ctx, &ws_id, &target_node_id, &id);
    Ok(())
}
...
pub fn update_edge(
    ...
    let latest_edge_id = if initial.invalid_at != 0 {
        ...
        ctx.db
            .kg_edge()
            .edge_group_id().filter(&initial.edge_group_id)
            .find(|e: &KgEdge| e.invalid_at == 0)
            .map(|e| e.id.clone())
            .unwrap_or(edge_id)
    } else {
        edge_id
...
pub fn get_edge_history(ctx: &ReducerContext, edge_group_id: String) -> Result<(), String> {
    ...
    // Pre-cleanup: remove stale results for this edge_group_id
    for old in ctx.db.edge_history_result().iter()
        .filter(|r| r.edge_group_id == edge_group_id)
        .collect::<Vec<_>>()
    {
        ctx.db.edge_history_result().id().delete(&old.id);
    }

    for edge in ctx
        .db
        .kg_edge()
        .edge_group_id().filter(&edge_group_id)
    {
...
/// Query an edge version as of a specific point in time.
///
/// Returns the edge version that was valid at `timestamp_micros`.
/// Uses the same `edge_history_result` table as `get_edge_history`,
/// with a single result row for the matching version.
#[reducer]
pub fn get_edge_as_of(ctx: &ReducerContext, edge_group_id: String, timestamp_micros: i64) -> Result<(), String> {
    let _account = require_auth(ctx)?;
    let _caller = ctx.sender().to_hex();
    ...
    // Find the edge version valid at the given timestamp
    let matching = ctx.db.kg_edge()
        .edge_group_id().filter(&edge_group_id)
        .filter(|e| e.valid_at <= timestamp_micros && (e.invalid_at == 0 || e.invalid_at > timestamp_micros))
        .collect::<Vec<_>>();

    if let Some(edge) = matching.last() {
        ctx.db.edge_history_result().insert(EdgeHistoryResult {
            id: uuid_v4_uniq(ctx, |id| ctx.db.edge_history_result().id().find(id).is_none(), 3),
            ...
            invalid_at: edge.invalid_at,
            version: edge.version,
            edge_group_id: edge.edge_group_id.clone(),
        });
    }
...
// ---------------------------------------------------------------------------
// Contradiction resolution (bi-temporal edge expiry)
// ---------------------------------------------------------------------------

/// Mark one or more KG edges as invalid (contradiction discovered).
///
/// Sets `invalid_at` on the specified edges to the current timestamp.
/// This uses the existing bi-temporal field on `KgEdge` — no schema
/// migration required.
///
/// The caller (Python SDK) is responsible for detecting the contradiction
/// via LLM or rule-based analysis; this reducer only materializes the
/// invalidation in the database.
///
/// # Arguments
/// - `workspace_id` — target workspace
/// - `edge_ids_json` — JSON array of edge IDs to invalidate, e.g. ``["id1","id2"]``
#[reducer]
pub fn resolve_edge_contradictions(
    ...
    let caller = ctx.sender().to_hex();
    let now = now_micros(ctx);
    let edge_ids: Vec<String> = serde_json::from_str(&edge_ids_json)
        .map_err(|e| format!("Invalid edge_ids_json: {}", e))?;

    for edge_id in &edge_ids {
        if let Some(mut edge) = ctx.db.kg_edge().id().find(edge_id) {
    ...
    change_event::log_change(
        ctx, &workspace_id, "resolve_edge_contradictions", "update",
        &edge_ids_json,
        &serde_json::json!({
            "count": edge_ids.len(),
...
pub fn seed_communities(ctx: &ReducerContext, workspace_id: String) -> Result<(), String> {
    ...
    let node_ids: Vec<(String, bool)> = ctx
        ...
        .filter(|n| n.community_id == 0)
        .map(|n| {
            let has_edge = ctx.db.kg_edge().workspace_id().filter(&workspace_id_clone)
                .take(crate::MAX_RESULTS).any(|e| {
...
#[table(accessor = pagerank_result)]
#[derive(Debug, Clone, serde::Serialize, serde::Deserialize)]
...
pub fn compute_community_hierarchy(
    ...
    > = std::collections::HashMap::new();
    for node in ctx.db.kg_node().workspace_id().filter(&workspace_id).take(crate::MAX_RESULTS) {
...
#[table(accessor = citation_result)]
#[derive(Debug, Clone, serde::Serialize, serde::Deserialize)]
...
pub fn get_citations(
    ...
    for existing in ctx.db.citation_result().workspace_id().filter(&workspace_id) {
        ctx.db.citation_result().id().delete(&existing.id);
    }

    for c in ctx.db.citation().workspace_id().filter(&workspace_id).take(crate::MAX_RESULTS) {
        if c.entity_id == entity_id && c.entity_type == entity_type {
...
mod tests {
    ...
    fn test_node_type_validation() {
        ...
        // Invalid types must be rejected by the valid set
        for t in ["invalid", "person", "place", "thing"] {
