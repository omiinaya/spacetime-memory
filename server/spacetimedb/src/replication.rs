...
#[table(accessor = replication_result)]
#[derive(Debug, Clone, serde::Serialize)]
    ...
    match entry.table_name.as_str() {
        "memory" => {
            ...
            let memory = Memory {
                ...
                source_url: data.get("source_url").and_then(|v| v.as_str()).unwrap_or("").to_string(),
            };
        ...
        "kg_node" => {
            ...
            let node = KgNode {
                ...
                created_at: data.get("created_at").and_then(|v| v.as_i64()).unwrap_or(now),
                source_document_id: data.get("source_document_id").and_then(|v| v.as_str()).unwrap_or("").to_string(),
                stale_since: data.get("stale_since").and_then(|v| v.as_i64()).unwrap_or(0),
            };
...
mod tests {
    ...
    // ── IncomingEntry parsing ─────────────────────────────────────────────

    #[test]
    fn test_incoming_entry_parse_insert() -> Result<(), String> {
        let json = r#"{"id":"e1","table_name":"memory","operation":"insert","record_id":"r1","data_json":"{}","created_at":1000}"#;
        let entry: IncomingEntry = serde_json::from_str(json)
            .map_err(|e| format!("valid JSON IncomingEntry should parse: {}", e))?;
        assert_eq!(entry.id, "e1");
        assert_eq!(entry.table_name, "memory");
        assert_eq!(entry.operation, "insert");
        assert_eq!(entry.record_id, "r1");
        assert_eq!(entry.data_json, "{}");
        assert_eq!(entry.created_at, 1000);
        Ok(())
    }

    #[test]
    fn test_incoming_entry_parse_update() -> Result<(), String> {
        let json = r#"{"id":"e2","table_name":"kg_node","operation":"update","record_id":"r2","data_json":"{\"label\":\"Updated\"}","created_at":2000}"#;
        let entry: IncomingEntry = serde_json::from_str(json)
            .map_err(|e| format!("valid update entry JSON should parse: {}", e))?;
        assert_eq!(entry.operation, "update");
        assert_eq!(entry.table_name, "kg_node");
        Ok(())
    }

    #[test]
    fn test_incoming_entry_parse_delete() -> Result<(), String> {
        let json = r#"{"id":"e3","table_name":"note","operation":"delete","record_id":"r3","data_json":"{}","created_at":3000}"#;
        let entry: IncomingEntry = serde_json::from_str(json)
            .map_err(|e| format!("valid delete entry JSON should parse: {}", e))?;
        assert_eq!(entry.operation, "delete");
        assert_eq!(entry.table_name, "note");
        Ok(())
    }

    #[test]
    fn test_incoming_entry_empty_data_json() -> Result<(), String> {
        let json = r#"{"id":"e4","table_name":"profile","operation":"insert","record_id":"r4","data_json":"","created_at":4000}"#;
        let entry: IncomingEntry = serde_json::from_str(json)
            .map_err(|e| format!("entry with empty data_json should parse: {}", e))?;
        assert_eq!(entry.data_json, "");
        Ok(())
    }

    #[test]
    fn test_incoming_entry_nested_data_json() -> Result<(), String> {
        let json = r#"{"id":"e5","table_name":"memory","operation":"insert","record_id":"r5","data_json":"{\"content\":\"Hello\",\"confidence\":0.9}","created_at":5000}"#;
        let entry: IncomingEntry = serde_json::from_str(json)
            .map_err(|e| format!("entry with nested data_json should parse: {}", e))?;
        let data: serde_json::Value = serde_json::from_str(&entry.data_json)
            .map_err(|e| format!("nested data_json should be valid JSON: {}", e))?;
        assert_eq!(data["content"], "Hello");
        let conf = data["confidence"].as_f64()
            .ok_or("confidence should be parseable as f64")?;
        assert!((conf - 0.9).abs() < 1e-6);
        Ok(())
    }

    #[test]
    fn test_incoming_entry_minimal() -> Result<(), String> {
        let json = r#"{"id":"e6","table_name":"","operation":"","record_id":"","data_json":"{}","created_at":0}"#;
        let entry: IncomingEntry = serde_json::from_str(json)
            .map_err(|e| format!("minimal entry JSON should parse: {}", e))?;
    ...
    #[test]
    fn test_incoming_entry_missing_fields_fails() {
        let json = r#"{"id":"e7","table_name":"memory"}"#;
        let result: Result<IncomingEntry, _> = serde_json::from_str(json);
        assert!(result.is_err());
    }

    // ── ReplicationPeer serialize ─────────────────────────────────────────

    #[test]
    fn test_replication_peer_serialize() -> Result<(), String> {
        let peer = ReplicationPeer {
            id: "p1".to_string(),
            workspace_id: "ws1".to_string(),
            name: "Peer One".to_string(),
            remote_url: "http://example.com:3001".to_string(),
            remote_db: "spacetimedb".to_string(),
        ...
        let json = serde_json::to_string(&peer)
            .map_err(|e| format!("ReplicationPeer should serialize to JSON: {}", e))?;
        assert!(json.contains("\"name\":\"Peer One\""));
        assert!(json.contains("\"is_active\":true"));
        Ok(())
    }

    // ── ReplicationLog serialize ──────────────────────────────────────────

    #[test]
    fn test_replication_log_serialize() -> Result<(), String> {
        let log = ReplicationLog {
            id: "l1".to_string(),
            workspace_id: "ws1".to_string(),
            table_name: "memory".to_string(),
        ...
        let json = serde_json::to_string(&log)
            .map_err(|e| format!("ReplicationLog should serialize to JSON: {}", e))?;
        assert!(json.contains("\"table_name\":\"memory\""));
        Ok(())
    }

    // ── ReplicationResult serialize ─────────────────────────────────────────

    #[test]
    fn test_replication_result_serialize() -> Result<(), String> {
        let result = ReplicationResult {
            id: "res1".to_string(),
            workspace_id: "ws1".to_string(),
            query_type: "status".to_string(),
        ...
        let json = serde_json::to_string(&result)
            .map_err(|e| format!("ReplicationResult should serialize to JSON: {}", e))?;
        assert!(json.contains("\"query_type\":\"status\""));
        Ok(())
    }
}
