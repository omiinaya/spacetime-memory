...
pub mod observation;
pub mod review;
pub mod reasoning_tier;
pub mod cognitive_op;
...
pub mod mib;
pub mod veracity;
pub mod embedder_alert;
pub mod tantivy_alert;
pub mod proxy_metrics;
pub mod embedder_metrics;
...
mod tests {
    ...
    #[test]
    fn test_format_uuid_v4_version_digit_is_4() -> Result<(), String> {
        let id = format_uuid_v4(0x0123456789abcdef, 0xfedcba9876543210);
        // The version digit is the first character of the third group.
        // Format: XXXXXXXX-XXXX-4XXX-XXXX-XXXXXXXXXXXX
        let parts: Vec<&str> = id.split('-').collect();
        let version_char = parts[2].chars().next().ok_or("third UUID group should have at least one char")?;
    ...
    #[test]
    fn test_format_uuid_v4_variant_digit_is_8_9_a_or_b() -> Result<(), String> {
        let id = format_uuid_v4(0x0123456789abcdef, 0xfedcba9876543210);
        let parts: Vec<&str> = id.split('-').collect();
        let variant_char = parts[3].chars().next().ok_or("fourth UUID group should have at least one char")?;
    ...
    #[test]
    fn test_format_uuid_v4_all_zero_input() -> Result<(), String> {
    ...
    #[test]
    fn test_format_uuid_v4_variant_depends_on_high_bits() -> Result<(), String> {
        ...
        for id in [&v0, &v1, &v2, &v3] {
            let parts: Vec<&str> = id.split('-').collect();
            let variant = parts[3].chars().next().ok_or("fourth UUID group should have at least one char")?;
    ...
    #[test]
    fn test_format_uuid_v4_output_matches_uuid_pattern() -> Result<(), String> {
        ...
        let re = regex::Regex::new(r"^[0-9a-f]{8}-[0-9a-f]{4}-4[0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$")
            .map_err(|e| format!("hardcoded UUID regex should compile: {}", e))?;
        assert!(re.is_match(&id), "UUID '{}' does not match expected 8-4-4-4-12 pattern", id);
        Ok(())
    }

    #[test]
    fn test_uuid_v7_format_is_standard() -> Result<(), String> {
        ...
        ).map_err(|e| format!("hardcoded UUID v7 regex should compile: {}", e))?;
...
/// Required stub for `getrandom` crate with `custom` feature on WASM.
/// The `spacetimedb` SDK depends on `getrandom` with the `custom` feature but
/// deliberately does NOT define `__getrandom_custom` (to prevent ThreadRng
/// misuse). Our module uses `ctx.rng()` (timestamp-seeded StdbRng) instead,
/// so this stub should never be called at runtime — it exists only to
/// satisfy the linker and eliminate the wasi `random_get` import that the
/// SpacetimeDB host does not provide.
#[cfg(target_arch = "wasm32")]
#[no_mangle]
pub extern "C" fn __getrandom_custom(_dest: *mut u8, _len: usize) -> u8 {
    0 // Return 0 (error) — indicates no random bytes available
}

// ---------------------------------------------------------------------------
// Re-exports — convenient wildcard imports for commonly used modules
// ---------------------------------------------------------------------------
pub use fact_triple::*;
