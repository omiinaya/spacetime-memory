...
pub fn apply_decay_inner(
    ...
    for mut mem in memories {
        ...
        let new_trust = if use_weibull {
            ...
            let k = weibull_shape.clamp(0.1_f64, 5.0_f64);
...
#[table(accessor = memory_recommendation)]
#[derive(Debug, Clone)]
...
pub fn recommend_memories(
    ...
    for mem in ctx.db.memory().workspace_id().filter(&workspace_id).filter(|m| m.is_active).take(crate::MAX_RESULTS)
    {
        let trust = mem.trust_score;
        let feedback = mem.feedback_count;
        let age_micros = now - mem.updated_at;
        ...
        let urgency = ((1.0_f64 - trust) * age_weight * feedback_penalty).min(1.0_f64);
...
#[table(accessor = peer_reputation_result)]
...
mod tests {
    ...
    fn test_linear_decay_formula() {
        // new_trust = trust_score * (1.0 - decay_rate * days_since_last_access)
        let trust_score = 1.0_f64;
        let decay_rate = 0.01_f64;
        let days = 10.0_f64;
        let expected = trust_score * (1.0_f64 - decay_rate * days);
        let actual = trust_score * (1.0_f64 - decay_rate * days);
        assert!((actual - expected).abs() < 1e-10_f64);
    }

    #[test]
    fn test_linear_decay_clamped_to_floor() {
        let trust_score: f64 = 1.0;
        let decay_rate: f64 = 0.01;
        let days: f64 = 200.0; // Way past max_days (90)
        let max_days: f64 = 90.0;
        let floor = 0.1;
    ...
    fn test_weibull_decay_formula() {
        ...
        let initial: f64 = 1.0;
        let t: f64 = 30.0; // days
        let lambda: f64 = 30.0; // scale
        let k: f64 = 0.6; // shape

        let exponent: f64 = -1.0 * (t / lambda).powf(k);
        let new_trust: f64 = initial * exponent.exp();
    ...
    fn test_weibull_decay_at_3_lambda() {
        ...
        let lambda: f64 = 30.0;
        let k: f64 = 0.6;
        let t: f64 = 3.0 * lambda; // 90 days

        let exponent: f64 = -1.0 * (t / lambda).powf(k);
        let new_trust: f64 = initial * exponent.exp();

        // At t = 3λ, trust ≈ 14.5% (1/e^((3)^0.6) = 1/e^1.933 ≈ 0.145)
        assert!((new_trust - 0.145).abs() < 0.02);
    }
    ...
    fn test_recommendation_urgency_calculation() {
        // urgency = (1.0 - trust_score) * age_weight * feedback_penalty
        let trust: f64 = 0.2;
        let age_weight: f64 = 1.5; // < 1 day
        let feedback_penalty: f64 = 1.0;
    ...
    fn test_recommendation_urgency_low_trust_old_memory() {
        let trust: f64 = 0.1;
        let age_weight: f64 = 0.8; // > 30 days
        let feedback_penalty: f64 = 1.0;
        let urgency = ((1.0 - trust) * age_weight * feedback_penalty).min(1.0);
        // (0.9 * 0.8) = 0.72
