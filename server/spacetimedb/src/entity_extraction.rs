...
use crate::entity_linking::EntityLink;
use crate::knowledge_graph::kg_edge;
use crate::knowledge_graph::kg_node;
use crate::knowledge_graph::KgNode;
...
fn is_proper_word(word: &str) -> bool {
    ...
    // Case checks are Unicode-aware so accented names ("José") pass;
    // scripts without case (CJK) can't be detected by this heuristic.
    match chars.next() {
...
fn entity_score(word: &str) -> i32 {
    ...
    // Common 2-char function words are noise even though they pass the
    // length gate — keeps them out without rejecting real 2-char entities
    // like "AI", "Go", "ML".
    let short_noise: &[&str] = &["an", "to", "of", "in", "on", "at", "by", "or", "as", "is", "it"];
    if short_noise.contains(&lower.as_str()) {
        return 0;
    }
    if lower.len() < 2 {
        return 0;
...
fn extract_single_words(text: &str) -> Vec<Mention> {
    ...
    for (i, word) in words.iter().enumerate() {
        ...
        // Guarded by the length check above; avoid panicking in production.
        ...
        if i + 1 < words.len() {
            ...
            if !next.is_empty() && next.chars().next().is_some_and(|c| c.is_ascii_uppercase()) {
                continue;
            }
        }
...
fn extract_people(text: &str) -> Vec<Mention> {
    ...
    for pair in words.windows(2) {
        // Never pair across a sentence boundary — "Google. Jane" is not a person.
        if pair[0].ends_with('.') || pair[0].ends_with('!') || pair[0].ends_with('?') {
            continue;
        }
        let first = pair[0].trim_end_matches(['.', ',', ';']);
        let second = pair[1].trim_end_matches(['.', ',', ';']);
...
/// Looser capitalization check for company/brand names: first char uppercase,
/// remaining chars alphanumeric. Unlike `is_proper_word`, allows internal
/// capitals so brand names like "OpenAI", "YouTube", "PayPal" are extracted.
...
fn extract_companies(text: &str) -> Vec<Mention> {
    ...
    for i in 0..words.len() {
        let current = words[i].trim_end_matches(['.', ',', ';']);
...
fn ensure_entity(
    ...
    // Check existing entity_link by exact name match (scoped to workspace)
    let mut link_id: Option<String> = None;
    for existing in ctx.db.entity_link().workspace_id().filter(workspace_id).take(crate::MAX_RESULTS) {
        if existing.entity_name.eq_ignore_ascii_case(&mention.name)
        {
            // Update usage count
    ...
    // Also ensure a KG node exists (scoped to workspace)
    let mut node_id: Option<String> = None;
    for existing in ctx.db.kg_node().workspace_id().filter(workspace_id).take(crate::MAX_RESULTS) {
        if existing.label.eq_ignore_ascii_case(&mention.name)
        {
...
#[table(accessor = entity_extraction_result)]
...
/// Run entity extraction on text and return results without auth check.
/// Shared between the `extract_entities` reducer and auto-extraction on store.
/// Returns (mentions, entity_ids).
pub(crate) fn auto_extract_entities(
    ctx: &ReducerContext,
    workspace_id: &str,
    text: &str,
    now: i64,
) -> (Vec<Mention>, Vec<String>) {
    // Run all extractors
    ...
    for i in 0..entity_ids.len() {
        for j in (i + 1)..entity_ids.len() {
            ...
            // Check if edge already exists (uses source_node_id btree index)
            let exists = ctx.db.kg_edge().source_node_id().filter(sub)
                .take(crate::MAX_RESULTS).any(|e: crate::knowledge_graph::KgEdge| e.target_node_id == *obj)
                || ctx.db.kg_edge().source_node_id().filter(obj)
                    .take(crate::MAX_RESULTS).any(|e: crate::knowledge_graph::KgEdge| e.target_node_id == *sub);
            if !exists {
                use crate::uuid_v7;
                let edge_id = uuid_v7(ctx);
                ctx.db.kg_edge().insert(crate::knowledge_graph::KgEdge {
                    id: edge_id.clone(),
                    workspace_id: workspace_id.to_string(),
                    source_node_id: sub.clone(),
                    target_node_id: obj.clone(),
    ...
    log::info!(
        "auto_extract_entities: {} mentions, {} entities for workspace {}",
        all_mentions.len(),
        entity_ids.len(),
        &workspace_id[..16.min(workspace_id.len())],
    );

    (all_mentions, entity_ids)
}

/// Full reducer for entity extraction.
/// Can also be called standalone by clients.
#[reducer]
pub fn extract_entities(ctx: &ReducerContext, workspace_id: String, text: String) -> Result<(), String> {
    ...
    // Use the shared auto-extraction logic (same as auto-extract on write)
    let (all_mentions, entity_ids) = auto_extract_entities(ctx, &workspace_id, &text, now);
    ...
    for old in ctx.db.entity_extraction_result().iter()
        .filter(|r| r.workspace_id == workspace_id)
        .collect::<Vec<_>>()
    {
        ctx.db.entity_extraction_result().id().delete(&old.id);
    }
...
mod tests {
    ...
    fn test_proper_word_single_char() {
        // Single-char words are rejected (len < 2) — "A" is noise in entity
        // extraction, not a valid entity candidate.
    ...
    fn test_entity_score_too_short() {
        assert_eq!(entity_score("a"), 0); // len < 2
        assert_eq!(entity_score(""), 0);
        // 2-char words pass the length gate unless they're function words
        // (handled by test_entity_score_short_words_zero) — "ab" is unknown
        // to both noise lists, so it scores as a candidate.
        assert_eq!(entity_score("ab"), 1);
    }
    ...
    fn test_is_proper_word_valid_ext() {
        ...
        assert!(!is_proper_word("A")); // len < 2 → rejected
    ...
    fn test_extract_single_words_skips_sentence_starters_ext() {
        let text = "Hello world. This is a test. People buy Apple products.";
        let words = extract_single_words(text);
        let names: Vec<_> = words.iter().map(|w| w.name.as_str()).collect();
        // "Hello" is first word of sentence - skipped
        // "This" is first word of sentence - skipped
        // "Apple" is mid-sentence (not after a sentence-ending period)
    ...
    #[test]
    fn test_extract_people_unicode_names() {
        let names = extract_people("Hello 山田太郎 and José García");
        assert!(names.is_empty() || names.iter().any(|n| n.name.contains("José") || n.name.contains("山田")));
    }

    #[test]
    fn test_extract_people_special_characters() {
        // Names with apostrophes, hyphens etc.
        let _names = extract_people("O'Brien and Smith-Jones and van der Waal");
        // At minimum should not panic
    }

    #[test]
    fn test_extract_people_empty_search() {
        let names = extract_people("");
        assert!(names.is_empty());
    }

    #[test]
    fn test_extract_people_very_large_content() {
        let large = "Alice and Bob ".to_string() + &"x".repeat(10_000);
        let names = extract_people(&large);
        // Should not panic and should find "Alice" and "Bob"
        assert!(names.iter().any(|n| n.name == "Alice") || names.is_empty());
    }

    #[test]
    fn test_extract_single_words_unicode() {
        // Unicode words should potentially be extracted
        let _words = extract_single_words("こんにちは 世界");
        // Should not panic at minimum
    }

    #[test]
    fn test_extract_single_words_special_characters() {
        let words = extract_single_words("hello-world!@#$%^&*()test");
        // Expected: "hello-world" might be split, "test" might be extracted
        assert!(words.is_empty() || !words.is_empty());
    }

    #[test]
    fn test_is_proper_word_unicode() {
        // Unicode-aware case checks: accented names pass.
        assert!(is_proper_word("José"));
        // CJK scripts have no case, so this heuristic can't detect them —
        // documented limitation (would need a CJK NER model).
        assert!(!is_proper_word("山田"));
    }

    #[test]
    fn test_is_proper_word_special_chars() {
        // Apostrophes are outside the strict title-case contract —
        // "O'Brien" has an uppercase 'B' after the apostrophe → rejected.
        assert!(!is_proper_word("O'Brien"));
    }

    #[test]
    fn test_is_proper_word_empty() {
        assert!(!is_proper_word(""));
        assert!(!is_proper_word(" "));
    }
