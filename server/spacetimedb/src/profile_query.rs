...
#[table(accessor = profile_context_result)]
#[derive(Debug, Clone)]
...
#[table(accessor = peer_summary_result)]
#[derive(Debug, Clone)]
...
#[table(accessor = directory_content_result)]
#[derive(Debug, Clone)]
...
pub fn get_profile_context(ctx: &ReducerContext, peer_id: String) -> Result<(), String> {
    ...
    // Pre-cleanup: remove stale results for this peer_id
    for old in ctx.db.profile_context_result().iter()
        .filter(|r| r.peer_id == peer_id)
        .collect::<Vec<_>>()
    {
        ctx.db.profile_context_result().id().delete(&old.id);
    }
...
pub fn get_peer_memory_summary(ctx: &ReducerContext, peer_id: String) -> Result<(), String> {
    ...
    // Pre-cleanup: remove stale results for this peer_id
    for old in ctx.db.peer_summary_result().iter()
        .filter(|r| r.peer_id == peer_id)
        .collect::<Vec<_>>()
    {
        ctx.db.peer_summary_result().id().delete(&old.id);
    }
...
pub fn search_directory_contents(
    ...
    // Pre-cleanup: remove stale results for this workspace_id + directory_path
    for old in ctx.db.directory_content_result().iter()
        .filter(|r| r.workspace_id == workspace_id && r.directory_path == directory_path)
        .collect::<Vec<_>>()
    {
        ctx.db.directory_content_result().id().delete(&old.id);
    }
...
mod tests {
    ...
    #[test]
    fn test_profile_context_result_initialization() {
        let result = ProfileContextResult {
            id: "pcr_001".to_string(),
            peer_id: "peer_abc".to_string(),
            static_facts_json: r#"{"name":"Alice"}"#.to_string(),
            dynamic_context_json: r#"{"status":"online"}"#.to_string(),
            preferences_json: r#"{"theme":"dark"}"#.to_string(),
            tags_json: r#"["admin","power-user"]"#.to_string(),
            query_text: String::new(),
            created_at: 1_000_000,
        };
        assert_eq!(result.peer_id, "peer_abc");
        assert_eq!(result.static_facts_json, r#"{"name":"Alice"}"#);
        assert!(result.query_text.is_empty());
        assert_eq!(result.created_at, 1_000_000);
    }

    #[test]
    fn test_peer_summary_result() {
        let summary = PeerSummaryResult {
            id: "psr_001".to_string(),
            peer_id: "peer_xyz".to_string(),
            memory_count: 42,
            insight_count: 7,
            session_count: 15,
            latest_activity: 2_000_000,
        };
        assert_eq!(summary.peer_id, "peer_xyz");
        assert_eq!(summary.memory_count, 42);
        assert_eq!(summary.insight_count, 7);
        assert_eq!(summary.session_count, 15);
        assert_eq!(summary.latest_activity, 2_000_000);
    }

    #[test]
    fn test_peer_summary_result_zero_counts() {
        let summary = PeerSummaryResult {
            id: "psr_002".to_string(),
        ...
        assert_eq!(summary.latest_activity, 0);
    }

    #[test]
    fn test_directory_content_result() {
        let result = DirectoryContentResult {
            id: "dcr_001".to_string(),
            workspace_id: "ws_001".to_string(),
            directory_path: "/root/knowledge".to_string(),
            directory_id: "dir_001".to_string(),
            subdirectory_ids_json: r#"["dir_002","dir_003"]"#.to_string(),
            memory_ids_json: r#"["mem_001","mem_002","mem_003"]"#.to_string(),
            created_at: 3_000_000,
        };
        assert_eq!(result.directory_path, "/root/knowledge");
        assert_eq!(result.directory_id, "dir_001");
        assert!(result.subdirectory_ids_json.contains("dir_002"));
        assert!(result.memory_ids_json.contains("mem_003"));
    }

    #[test]
    fn test_directory_content_result_empty() {
        let result = DirectoryContentResult {
            id: "dcr_002".to_string(),
            workspace_id: "ws_002".to_string(),
            directory_path: "/empty".to_string(),
            directory_id: "dir_empty".to_string(),
            subdirectory_ids_json: "[]".to_string(),
            memory_ids_json: "[]".to_string(),
            created_at: 0,
        };
        assert_eq!(result.subdirectory_ids_json, "[]");
        assert_eq!(result.memory_ids_json, "[]");
