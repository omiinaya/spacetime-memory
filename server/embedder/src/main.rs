...
fn compute_embedding(
    ...
    // SimplePlan has run(self: &Arc<Self>, ...) so we get Arc out of Mutex
...
async fn embed(
    ...
    for t in &texts {
        all_embeddings.push(compute_embedding(&state.model, &state.tokenizer, t));
    }
...
async fn main() {
    ...
    let model_path = std::env::var("MODEL_PATH")
        .unwrap_or_else(|_| "model/all-MiniLM-L6-v2.onnx".to_string());
    ...
    let dummy_type: Vec<i64> = vec![0i64, 0, 0];
    let di = Tensor::from_shape(&[1, 3], &dummy_ids).unwrap();
    let dm = Tensor::from_shape(&[1, 3], &dummy_mask).unwrap();
    let dt = Tensor::from_shape(&[1, 3], &dummy_type).unwrap();
    {
        let mut m = model.run(tvec!(di.into(), dm.into(), dt.into())).unwrap();
        let dummy_result = m.remove(0);
        let sv = dummy_result.to_plain_array_view::<f32>().unwrap();
        let dimension = sv.shape()[2];
        println!("Embedder: ready. dimension={} model={}", dimension, model_path);
    }
    ...
    let state = Arc::new(AppState {
        ...
        dimension: 384, // will fix once model is loaded
    });
