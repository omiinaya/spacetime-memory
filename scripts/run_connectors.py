...
def _build_telegram(config: dict, registry: ConnectorRegistry) -> None:
    """Register a Telegram connector from config."""
    required = ("token", "workspace_id")
    if not all(k in config for k in required):
        print("  [config] telegram: missing token or workspace_id")
        return
    registry.register(
        "telegram",
        TelegramConnector(
            ...
            workspace_id=config["workspace_id"],
            peer_id=config.get("peer_id", "telegram-bot"),
            include_callback_queries=config.get("include_callback_queries", True),
        ),
    )
