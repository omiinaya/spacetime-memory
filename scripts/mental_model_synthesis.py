...
def _sql_param(query_template: str, *args: Any) -> list[dict[str, Any]]:
    ...
    for arg in args:
        if arg is None:
            ...
            escaped.append(f"'{str(arg).replace(chr(39), chr(39)+chr(39))}'")
    buf: list[str] = []
    for i, p in enumerate(parts):
        buf.append(p)
        if i < len(escaped):
            buf.append(escaped[i])
    return _sql("".join(buf))
...
def get_pending_models(all_flag: bool = False) -> list[dict[str, Any]]:
    ...
    if cutoff > 0:
        rows = _sql_param(
            "SELECT * FROM mental_model WHERE status = 'pending' AND updated_at > ? ORDER BY created_at ASC",
            cutoff,
        )
    else:
        rows = _sql("SELECT * FROM mental_model WHERE status = 'pending' ORDER BY created_at ASC")
    return rows
...
def get_memories(memory_ids: list[str]) -> list[dict[str, Any]]:
    ...
    rows = _sql_param(
        f"SELECT id, content, summary, memory_type FROM memory WHERE id IN ({placeholders})",
        *memory_ids,
    )
    return rows
