#!/usr/bin/env python3
"""Focused 20-iteration semantic search benchmark — no LLM entity extraction."""
import os, json, statistics, sys, time
...
print("=" * 60)
print("Semantic Search Latency Benchmark (focused)")
print("=" * 60)
...
# Use unique workspace each run to avoid auth conflicts
...
# Seed 50 memories — disable LLM entity extraction by ensuring no OPENAI_API_KEY for LLM
os.environ.pop("OPENAI_API_KEY", None)
os.environ["OPENAI_API_KEY"] = "sk-no-auth-needed"  # embedder only, not LLM
...
for i in range(50):
    ...
    # Sleep briefly to let async indexing settle
    time.sleep(0.01)
print("  Done seeding.")

# Let Tantivy catch up
time.sleep(2)
...
# 1. Embed-only benchmark (5 iterations)
def do_embed():
...
out_path = "benchmark_results_latest.json"
