#!/usr/bin/env python3
"""LoCoMo Benchmark for Spacetime-Memory — v2 improved retrieval.

Key improvements over v1:
  1. Store turns WITHOUT metadata prefix (cleaner embeddings = better semantic search)
  2. Use hybrid search (semantic + keyword) with cross-encoder reranking
  3. Include temporal metadata SEPARATELY (not in content) for post-sort
  4. Better QA prompts for temporal vs factual questions
  5. Robust judge with multiple fallback strategies
  6. Wait for Tantivy indexing before QA phase
"""
...
# Fallback: load key from ~/.hermes/.env
...
def llm_judge(question: str, expected: str, answer: str) -> dict:
    """Judge answer correctness with LLM + fallback."""
...
def ingest_conversation(client: Client, workspace_id: str, conversation: dict) -> int:
    """Ingest all turns via store_batch for fast ingestion (5s vs 12min)."""
...
def run_qa(conversation: dict, workspace_id: str, client: Client) -> list[dict]:
    """Run QA queries. Uses hybrid search + chronological context for temporal questions."""
    ...
    for i, qa in enumerate(qa_list):
        ...
        # Search with hybrid (semantic + keyword + cross-encoder)
        try:
            ...
            results.append({
                ...
                "actual_answer": f"SEARCH ERROR: {e}", "is_correct": False,
                "category": category, "reasoning": f"Search failed: {e}", "evidence": [],
            })
            continue
        ...
        for sr in search_results[:30]:
            content = sr.get("content", sr.get("memory_content", ""))[:500]
            score = sr.get("score", 0)
            meta = _extract_session_info(sr)
            if content:
                scored_items.append({
        ...
        # Sort: temporal questions by chronological order, others by score
        ...
        # Different prompts for temporal vs factual
        if category == 2:
            prompt = f"""You are a precise memory system. Based ONLY on the chronologically-ordered conversation excerpts below, answer the question about WHEN something happened. Give the specific date or time period.
...
If you cannot find the exact answer, say "I don't know".""" 
        else:
...
If you cannot find the exact answer, say "I don't know"."""
        ...
        try:
            ...
            actual_answer = (ans_data["choices"][0]["message"]["content"] or "").strip()
        ...
        results.append({
            "question": question, "expected_answer": expected_answer,
            "actual_answer": actual_answer, "is_correct": judge_result["is_correct"],
            "category": category, "reasoning": judge_result["reasoning"],
...
def main():
    ...
    parser.add_argument("--output", type=str, default="benchmark_results_locomo.json")
    ...
    print("  LoCoMo Benchmark v2 for Spacetime-Memory", file=sys.stderr)
    print("=" * 60, file=sys.stderr)
    ...
    if not db_id:
        ...
        sys.exit(1)
    # Use pre-stored token from env when available (keeps same identity across runs)
    ...
    try:
        import secrets
        client._call("register", [f"locomo-v2-{secrets.token_hex(4)}", "lmeval2026", "benchmarkpass"])
    except Exception:
        pass
