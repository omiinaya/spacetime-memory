...
- Also measures sub-components: embed, keyword fallback, full pipeline

Usage:
  python3 scripts/benchmark_semantic_only.py

Environment: .env file at repo root expected.
"""

import os, json, statistics, sys, time, uuid as _uuid
...
# Use the standard benchmark workspace
ws_id = "bench-ws-1"
try:
    c.create_workspace(name=ws_id)
except RuntimeError:
    pass
...
if mem_count < 50:
    print(f"Seeding {50 - mem_count} more memories...")
    for i in range(50 - mem_count):
        c.store(ws_id,
                content=f"Benchmark test memory number {i} for keyword and semantic searches. Testing retrieval quality verification.",
...
# 1. Baseline: just embed a query (no STDB interaction)
def do_embed():
    c._embed("test query for benchmark verification " * 3)
run("embed-only (bge-m3)", do_embed, n=min(N, 5))

# 2. Keyword-only search (no semantic, no enrichment)
run("search.keyword (top-5)", lambda: c.search(ws_id, "test memory retrieval", limit=5, semantic=False), n=N)

# 3. Semantic search (the one we care about — was 7.5s, should be ~2.5s)
...
out_path = f"benchmark_results_latest.json"
