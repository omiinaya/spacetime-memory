...
The raw Tantivy sidecar search is **2.2ms p50 / 4.7ms p90** on this run (direct HTTP API with a persistent keep-alive client; 1.3–4.5ms in the earlier same-day runs — all sub-10ms, environment-sensitive). The SDK `c.search(semantic=False)` path adds two STDB HTTP round trips (`check_workspace_access` + `_enrich_entities_json`) which dominate on this loaded host (load average ~39–42, concurrent longmemeval chain benchmark serving the same DB) — hence 77.8ms p50 / 110.3ms p90 end-to-end. Tantivy OFF falls back to the SDK's `_keyword_fallback` (cross-process STDB `_query` + client-side BM25): a slow path under load, min 105ms / p50 320ms / p90 6.4s (STDB worker stalls), mean 1768ms. Tantivy eliminates that tail: deterministic 77.8ms p50 / 110.3ms p90 vs a 6.4-second p90 on the fallback — a **~58× p90 tail-latency improvement** (4.1× at p50). The p50 speedup varies with load (24.1× in the heavy-load run when the fallback's STDB query was cold, 1.5× when warm, 4.1× here); the p90 tail elimination is the stable win across every run.
