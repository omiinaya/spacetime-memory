...
|--------|---------:|---------:|----------:|---------:|---------:|--------:|
| keyword (Tantivy ON, SDK c.search) | 77.8 | 110.3 | 81.5 | 41.0 | 164.5 | — |
| keyword (Tantivy OFF, SDK fallback) | 319.7 | 6430.1 | 1768.0 | 105.4 | 6566.7 | **4.1× p50 / ~58× p90** |

The raw Tantivy sidecar search is **2.2ms p50 / 4.7ms p90** on this run (direct HTTP API with a persistent keep-alive client; 1.3–4.5ms in the earlier same-day runs — always sub-10ms, environment-sensitive). The SDK `c.search(semantic=False)` path adds two STDB HTTP round trips (`check_workspace_access` + `_enrich_entities_json`), which dominate end-to-end latency (77.8ms p50 / 110.3ms p90 here — the STDB worker was also serving a concurrent longmemeval chain benchmark on the same DB). Tantivy OFF falls back to the SDK's `_keyword_fallback` (cross-process STDB `_query` + client-side BM25): 319.7ms p50 but with a **multi-second tail** (p90 6.4s, max 6.6s, mean 1768ms — the STDB worker stalls under concurrent load). Tantivy eliminates that tail: deterministic 77.8ms p50 / 110.3ms p90 vs a 6.4-second p90 on the fallback — a **~58× p90 tail-latency improvement** (4.1× at p50). The p50 speedup varies with load (24.1× in the heavy-load run when the fallback's STDB query was cold, 1.5× when warm, 4.1× here); the p90 tail elimination is the stable win across every run.
...
**Tantivy's contribution in this re-run:** deterministic latency (77.8ms p50 / 110.3ms p90 SDK path; 2.2ms p50 direct API) vs the fallback's 105ms–6.6s spread (p90 6.4s), and deterministic quality parity (P@5=61.3% / MRR=0.608 — byte-identical to the 06:46Z, 11:18Z and 17:06Z edition runs) confirming the BM25 sidecar produces equivalent rankings. Absolute SDK-path numbers are environment-sensitive (host load ~39–42 during this run plus a concurrent longmemeval chain benchmark); the raw-sidecar speed (sub-10ms in every run), the p90 tail elimination (~58× here, ~81–85× in the earlier runs), and the quality parity are stable across all editions.
...
1. **Tantivy BM25 sidecar** — deterministic keyword search: direct API 2.2ms p50 / 4.7ms p90, SDK path 77.8ms p50 / 110.3ms p90, vs the fallback's 319.7ms p50 / 6.4s p90 (multi-second tail; ~58× p90 improvement, 1.5×–24.1× p50 depending on host load). Quality parity with client-side BM25 fallback (deterministic across all four re-runs: 61.3%/0.608 vs 62.7%/0.703). The SDK path comparison is the realistic end-to-end metric users experience; the fallback is only nominally functional because of the `_query` bug.
...
4. **Tantivy is the only functional keyword path** while the `_query` bug (workspace_id filter returns 0 rows) remains unfixed — the SDK fallback returns 0 results for any specific workspace, and its latency is slow and tail-heavy (105ms floor → 6.4s p90 under load).
