  ...
  const handleSearch = useCallback(async () => {
    ...
    try {
      await callReducer('hybrid_search', [
        ...
        20, // limit
        '["keyword","temporal"]',
        0, // cache_ttl_ms = 0 (no cache)
        true, // compound = True (Bayesian veracity boost on recall)
      ]);
      ...
      const items = parseSqlResponse<SearchResult>(res);

      // Enrich with veracity tiers from the memory table
      const tierMap = new Map<string, string>();
      if (items.length > 0) {
        const entityIds = items
          .filter(r => r.entity_type === 'memory')
          .map(r => r.entity_id)
          .filter(Boolean);
        if (entityIds.length > 0) {
          const tierRes = await executeSql(
            `SELECT id, tier FROM memory WHERE id IN (${entityIds.map(id => `'${_esc(id)}'`).join(',')})`,
          );
          const tierRows = parseSqlResponse<{ id: string; tier: string }>(tierRes);
          for (const row of tierRows) {
            tierMap.set(row.id, row.tier);
          }
        }
      }
      ...
      setResults(items.map(r => ({
        ...r,
        veracity_tier: tierMap.get(r.entity_id) || undefined,
      })));
    } catch (err) {
                ...
                <div key={r.id} className="rounded-lg border border-border p-3">
                  <div className="flex items-start justify-between gap-2">
                    <div className="flex items-center gap-1.5">
                      ...
                      {r.veracity_tier && (
                        <VeracityBadge tier={r.veracity_tier} />
                      )}
                    </div>
