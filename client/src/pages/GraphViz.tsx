...
interface KGNodeRow {
  ...
  workspace_id: string;
  label: string;
  node_type: string; // 'code', 'concept', 'entity', 'document', 'topic'
  summary: string;
...
export default function GraphViz() {
  ...
  const loadGraphData = useCallback(async () => {
    ...
    try {
      ...
      const graphNodes: GraphNode[] = fetchedNodes.map((n: KGNodeRow) => {
        if (n.community_id != null) communities.add(n.community_id);
        return {
          id: n.id,
          label: n.label,
      ...
      const graphEdges: GraphEdge[] = fetchedEdges
        .filter((e: KGEdgeRow) => nodeMap.has(e.source_node_id) && nodeMap.has(e.target_node_id))
        .map((e: KGEdgeRow) => ({
          id: e.id,
