...
interface MockAuth {
  account?: AccountRow;
  /** Force an unauthenticated auth status so AuthPage renders (E2E). */
  status?: 'needs_account' | 'login';
}
...
export function AuthProvider({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    ...
    if (mock?.status) {
      // Force an unauthenticated status so AuthPage renders (register/login flows).
      setStatus({ type: mock.status });
      setMyIdentity('e2e-mock-identity');
      return;
    }
  }, []);
  ...
  useEffect(() => {
    ...
    const checkIdentity = () => {
      ...
      if (identity) {
        setMyIdentity(identity.toHexString().toLowerCase());
      }
    };
