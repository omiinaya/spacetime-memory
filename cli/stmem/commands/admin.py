...
def admin_init(identity: str, token: str | None) -> None:
    ...
    if token:
        client.token = token
    with _root.console.status("Setting initial admin..."):
        client._call("set_initial_admin", [identity])
    _root.console.print(f"[green]Initial admin set for identity {identity[:16]}...[/green]")
...
def admin_promote(identity: str, token: str | None) -> None:
    ...
    if token:
        client.token = token
    with _root.console.status(f"Promoting identity {identity[:16]}... to admin..."):
        client._call("promote_admin", [identity])
    _root.console.print(f"[green]Identity {identity[:16]}... promoted to admin.[/green]")
...
def admin_demote(identity: str, token: str | None) -> None:
    ...
    if token:
        client.token = token
    with _root.console.status(f"Demoting identity {identity[:16]}... to user..."):
        client._call("demote_admin", [identity])
    _root.console.print(f"[green]Identity {identity[:16]}... demoted to user.[/green]")
...
def admin_list(token: str | None) -> None:
    ...
    if token:
        client.token = token
    with _root.console.status("Fetching admin list..."):
    ...
    if not rows:
        _root.console.print("[yellow]No admin accounts found.[/yellow]")
        return
