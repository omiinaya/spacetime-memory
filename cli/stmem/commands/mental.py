...
def mental_list(status: str) -> None:
    ...
    client = _sdk_client()
    if status:
        with _root.console.status("Fetching mental models..."):
...
def mental_create(workspace_id: str, memory_ids: str) -> None:
    ...
    with _root.console.status("Creating mental model..."):
...
def mental_synthesize(all_flag: bool, dry_run: bool) -> None:
    ...
    if not os.path.exists(script_path):
        _root.console.print(f"[red]Error: {script_path} not found.[/red]")
        sys.exit(1)
    ...
    _root.console.print(f"[dim]Running: {' '.join(cmd)}[/dim]")
    result = subprocess.run(cmd, capture_output=False)
    if result.returncode != 0:
        _root.console.print(f"[red]Synthesis script exited with code {result.returncode}[/red]")
...
def mental_get(id: str) -> None:
    ...
    client = _sdk_client()
    with _root.console.status(f"Fetching mental model '{id[:16]}...'..."):
        rows = client._sql(
            f"SELECT * FROM mental_model WHERE id = '{_esc(id)}'"
        )
    if not rows:
        _root.console.print(f"[yellow]Mental model '{id[:16]}...' not found.[/yellow]")
        return
    print_json(rows[0])
