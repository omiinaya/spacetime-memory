...
def decay_set_linear(workspace_id: str, rate: float, max_days: int) -> None:
    ...
    client = _sdk_client()
    with _root.console.status(f"Applying linear decay to workspace '{workspace_id[:12]}...'..."):
...
def decay_set_weibull(workspace_id: str, shape: float, scale: float) -> None:
    ...
    """
    client = _sdk_client()
    with _root.console.status(f"Applying Weibull decay to workspace '{workspace_id[:12]}...'..."):
...
def decay_show(workspace_id: str) -> None:
    ...
    if config:
        ...
        _root.console.print("[yellow]No decay config set (defaults: linear, 0.5%/day, 90 day max)[/yellow]")
...
def decay_run(workspace_id: str) -> None:
    ...
    with _root.console.status(f"Running {model} decay on workspace '{workspace_id[:12]}...'..."):
        if model == "weibull":
