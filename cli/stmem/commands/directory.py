...
def directory_list(ctx: click.Context, directory_id: str, watch: bool) -> None:
    ...
    def _run() -> list[dict[str, Any]]:
        with _root.console.status(f"Listing directory '{directory_id[:16]}...'..."):
...
def directory_tree(workspace_id: str, root_directory_id: str) -> None:
    ...
    with _root.console.status(f"Traversing tree from '{root_directory_id[:16]}...'..."):
...
def directory_create(workspace_id: str, name: str, path: str,
    ...
    with _root.console.status(f"Creating directory '{name}'..."):
...
def directory_link(directory_id: str, memory_id: str, workspace_id: str) -> None:
    ...
    with _root.console.status(f"Linking memory '{memory_id[:16]}...' to directory..."):
...
def directory_unlink(directory_id: str, memory_id: str) -> None:
    ...
    with _root.console.status(f"Unlinking memory '{memory_id[:16]}...' from directory..."):
