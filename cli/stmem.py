...
def encryption_init(workspace_id: str) -> None:
    ...
    """
    client = _sdk_client()
    with console.status(f"Initialising encryption for workspace '{workspace_id[:16]}...'..."):
...
def encryption_enable(workspace_id: str) -> None:
    ...
    client = _sdk_client()
    with console.status(f"Enabling encryption for workspace '{workspace_id[:16]}...'..."):
...
def encryption_disable(workspace_id: str) -> None:
    ...
    """
    client = _sdk_client()
    with console.status(f"Disabling encryption for workspace '{workspace_id[:16]}...'..."):
...
def encryption_rotate(workspace_id: str) -> None:
    ...
    """
    client = _sdk_client()
    with console.status(f"Rotating encryption key for workspace '{workspace_id[:16]}...'..."):
...
def encryption_encrypt_existing(workspace_id: str) -> None:
    ...
    """
    client = _sdk_client()
    with console.status(f"Encrypting existing memories in workspace '{workspace_id[:16]}...'..."):
...
def encryption_decrypt_memory(memory_id: str) -> None:
    ...
    client = _sdk_client()
    with console.status(f"Fetching decrypted memory '{memory_id[:16]}...'..."):
...
def detect_patterns_cmd(ctx: click.Context, workspace_id: str, limit: int,
    ...
    """
    result = {}
    with console.status(f"Analyzing workspace '{workspace_id}'..."):
    ...
    if not result or result.get("total_memories", 0) == 0:
        console.print("[yellow]No memories found to analyze.[/yellow]")
        return
    ...
    # Temporal Clusters
    if clusters:
        console.print(f"[bold cyan]Temporal Clusters ({len(clusters)}):[/bold cyan]")
        ...
        console.print("[dim]No temporal clusters found.[/dim]")
    ...
    if terms:
        ...
        console.print("[dim]No frequent terms found.[/dim]")
    ...
    if co_occurs:
        ...
        console.print("[dim]No co-occurrences found.[/dim]")
    ...
    def _run() -> dict[str, Any] | None:
        ...
        with console.status("Fetching replication status..."):
            ...
            rows = client._sql(
                ...
                "AND workspace_id = '{}' "
                "ORDER BY created_at DESC LIMIT 1".format(_esc(ws_id))
            )
        if not rows:
